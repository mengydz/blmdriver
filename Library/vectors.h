...
#define LengthVector3(a)	(sqrt((a).x * (a).x + (a).y * (a).y + (a).z *(a).z))

#define VECTOR_PLUS_NUM(resultV,orignV,num)				\
	do{												\
		resultV.x = orignV.x * num;					\
		resultV.y = orignV.y * num;					\
		resultV.z = orignV.z * num;					\
	}while(0)
